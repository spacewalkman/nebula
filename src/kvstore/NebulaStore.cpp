...
bool NebulaStore::init() {
    ...
    raftService_ = raftex::RaftexService::createService(ioPool_, workers_, raftAddr_.second);
            ...
            for (auto &dir : dirs) {
                ...
                try {
                    ...
                    if (spaceIt == this->spaces_.end()) {
                        ...
                        spaceIt =
                            this->spaces_.emplace(spaceId, std::make_unique<SpacePartInfo>()).first;
                    }
...
std::unique_ptr<KVEngine> NebulaStore::newEngine(GraphSpaceID spaceId, const std::string &path) {
    if (FLAGS_engine_type == "rocksdb") {
        ...
        return std::make_unique<RocksEngine>(spaceId, path, options_.mergeOp_, options_.cfFactory_);
    } else {
...
std::shared_ptr<Part> NebulaStore::newPart(GraphSpaceID spaceId,
    ...
    auto part =
        std::make_shared<Part>(spaceId,
                               partId,
                               raftAddr_,
                               folly::stringPrintf("%s/wal/%d", engine->getDataRoot(), partId),
                               engine,
                               ioPool_,
                               bgWorkers_,
                               workers_,
...
void NebulaStore::asyncPut(GraphSpaceID spaceId, PartitionID partId, KV keyValue, KVCallback cb) {
    ...
    part->asyncPut(std::move(keyValue.first), std::move(keyValue.second), std::move(cb));
}
