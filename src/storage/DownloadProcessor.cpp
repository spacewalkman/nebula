...
void DownloadProcessor::process(const cpp2::DownloadReq &req) {
  ...
  const auto &hdfsUtils =
      HdfsUtils::getInstance(FLAGS_hdfs_namenode, FLAGS_namenode_port, getThreadManager());

  std::vector <cpp2::PartitionID> partitionToDownloads(std::move(req.get_partIds()));
  if (partitionToDownloads.empty()) {
    auto partSubDirs = hdfsUtils->listSubDirs(req.get_source_dir(), "\\d+");
    std::move(partSubDirs->begin(),
              partSubDirs->end(),
  ...
  for (auto partId : partitionToDownloads) {
    ...
    std::move(tasksForOnePart.begin(), tasksForOnePart.end(), std::back_inserter(allTasks));
  }

  //TODO：getThreadManager()从哪里来？
  auto allResultFutures = folly::collectAll(allTasks)
      .via(getThreadManager()).then([this](std::vector <StatusOr<std::string>> results) {
    std::for_each(results.begin(), results.end(), [](StatusOr <std::string> &st) {
      if (!st.status().ok()) {
        this->pushResultCode(cpp2::ErrorCode::E_DOWNLOAD,
...
cpp2::PartitionID DownloadProcessor::extractPartIdFromFileName(folly::StringPiece fileName) {
