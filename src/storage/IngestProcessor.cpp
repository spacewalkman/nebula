...
void IngestProcessor::process(const cpp2::IngestReq& req) {
    ...
    folly::collectAll(allTasks)
        .via(getThreadManager()).then([this](std::vector<StatusOr<std::string>> result) {
      std::for_each(result.begin(), result.end(), [](StatusOr<std::string>& st) {
        if (!st.status().ok()) {
